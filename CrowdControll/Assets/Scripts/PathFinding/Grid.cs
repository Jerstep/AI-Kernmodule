...
public class Grid : MonoBehaviour
{
    ...
    Dictionary<int, int> walkebleRegionsDictionary = new Dictionary<int, int>();
    LayerMask walkebleMask;
    ...
    private void Awake()
    {
        ...
        {
            walkebleMask.value = walkebleMask |= region.terrainMask.value;
            walkebleRegionsDictionary.Add((int)Mathf.Log(region.terrainMask.value, 2), region.terrainPenalty);
        }
    ...
    private void CreateGrid()
    {
        ...
        for(int x = 0; x < gridSizeX; x++)
        {
            for(int y = 0; y < gridSizeY; y++)
            {
                ...
                RaycastHit hit;
                if(Physics.Raycast(ray, out hit, 100, walkebleMask))
                {
                    walkebleRegionsDictionary.TryGetValue(hit.collider.gameObject.layer, out movementPenalty);
                }
                if(!walkeble)
                {
                ...
                grid[x, y] = new Node(walkeble, worldPoint, x, y, movementPenalty);
            }
        }
    ...
    void BlurPenaltyMap(int blurSize)
    {
        ...
        for(int y = 0; y < gridSizeY; y++)
        {
            for(int x = -kernelExtents; x <= kernelExtents; x++)
            {
                ...
                penaltysHorizontalPass[0, y] += grid[sampleX, y].movementPenalty;
            }
            for(int x = 1; x < gridSizeX; x++)
            {
                ...
                penaltysHorizontalPass[x, y] = penaltysHorizontalPass[x - 1, y] - grid[removeIndex, y].movementPenalty + grid[addIndex, y].movementPenalty;
            }
        }
        for(int x = 0; x < gridSizeX; x++)
        {
    ...
    private void OnDrawGizmos()
    {
        ...
        if(grid != null && displayGridGizmos)
        {
            foreach (Node n in grid)
            {
                ...
                Gizmos.color = (n.walkeble) ? Gizmos.color : Color.red;
